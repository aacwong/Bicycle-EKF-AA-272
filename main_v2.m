...
plot(t+t0,sqrt(squeeze(P(1,1,:))),'DisplayName','\sigma x')
plot(t(1:end)+t0,sqrt(squeeze(P(2,2,1:end))),'DisplayName','\sigma y')
plot(t+t0,sqrt(squeeze(P(3,3,:))),'DisplayName','\sigma z')
title('X,Y,Z standard deviations'); ylabel('(m)'); ylim([0,10])
legend
a2=subplot(3,1,2); hold on;
plot(wheelspeed_measurements.timestamp, wheelspeed_measurements.omega);
title('wheel speed measurements'); ylabel('rad/sec')
a3=subplot(3,1,3); hold on;
scatter(gnss_measurements.timestamp, gnss_measurements.pseudorange_sigma);
linkaxes([a1,a2,a3],'x')
title('\rho reported \sigma'); ylabel('m'); xlabel('time since unix epoch (s)')
%
%% approximate rotation matrix from ecef to enu
...
function plot_states(t,X,P)
    ...
    for i=1:8
        ...
        plot(t-t(1),3*squeeze(P(i,i,:))' + X(i,:)-X(i,1));
        plot(t-t(1),-3*squeeze(P(i,i,:))' + X(i,:)-X(i,1));
        title(labels{i})
    end
